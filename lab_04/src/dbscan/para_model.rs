...
impl ParallelModel {
    ...
    fn make_cells(&mut self) {
        let mut binding = self.dataset.lock().unwrap();
        let max = binding.get_data()[self.focus.1].iter().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
        let min = binding.get_data()[self.focus.1].iter().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap();
        let disty = max - min;
        ...
        self.pairs = vx.iter().cloned().zip(vy.iter().cloned()).collect::<Vec<_>>();
        ...
        for i in 0..self.pairs.len() {
            if self.pairs[i].0 > (cur_strip + 1) as f64 * (eps_len) + minx {
                parallel_sort(&mut self.pairs[self.strips[cur_strip].point_id..i], &pairs_compare_by_y, self.threads_cnt);
                cur_strip += 1;
                self.strips.push(Strip{
                    id: cur_strip,
                    amount: 0,
            ...
            for slice in (0..n).step_by(chunk_size) {
                ...
                scope.spawn(move |_| {
                    ...
                    for i in slice..std::cmp::min(slice+chunk_size, n) {
                        let ((_, yc1), (_, yc2)) = (self_arc.pairs[self_arc.cells[i].point_id], self_arc.pairs[self_arc.cells[i].point_id + self_arc.cells[i].amount - 1]);
                        let (min_strip, max_strip) = (self_arc.cells[i].strip_id.saturating_sub(1), std::cmp::min(self_arc.cells[i].strip_id + 2, self_arc.strips.len() - 1));
                        for k in min_strip..max_strip {
                            if k == self_arc.cells[i].strip_id {
                                for n in i.saturating_sub(2)..std::cmp::min(i+2,n) {
                                    ...
                                    let ((_, y1), (_, y2)) = (self_arc.pairs[self_arc.cells[n].point_id], self_arc.pairs[self_arc.cells[n].point_id + self_arc.cells[n].amount - 1]);
                            ...
                            for j in 0..self_arc.strips[k].cells_amount {
                                ...
                                let ((_, y1), (_, y2)) = (self_arc.pairs[self_arc.cells[c + j].point_id], self_arc.pairs[self_arc.cells[c + j].point_id + self_arc.cells[c + j].amount - 1]);
                                if (y1 - yc2).abs() < self_arc.eps ||
                                    (y2 - yc1).abs() < self_arc.eps ||
                                    (y1 - yc1).abs() < self_arc.eps ||
                                    (y2 - yc2).abs() < self_arc.eps {
    ...
    fn mark_core(&mut self) {
        ...
        self.visited = visited.lock().unwrap().clone();
    }
                        ...
                        if cell.amount >= self_arc.min_pts {
                            for neighbor in &cell.neighbors {
                                if self_arc.cells[*neighbor].amount >= cell.amount {
                                    ...
                                    if cell.id > self_arc.cells[*neighbor].id && self_arc.cells[*neighbor].amount >= self_arc.min_pts && union_find.find(cell.id) != union_find.find(self_arc.cells[*neighbor].id) {
                                        union_find.union(i, *neighbor);
                                    }
                                }
                            }
                            ...
                            if cell.amount >= self_arc.min_pts {
                                for p in cell.point_id..(cell.point_id + cell.amount) {
                                    if self_arc.visited[p] {
                                        clusters[p] = union_find.find(j) as i32;
                ...
                // scope.spawn(move |_| {
                    ...
                    if cell.amount < self.min_pts {
                        for i in cell.point_id..(cell.point_id+cell.amount) {
                            for neighbor in &cell.neighbors {
                                for p in self.cells[*neighbor].point_id..(self.cells[*neighbor].point_id+self.cells[*neighbor].amount) {
                                    if self.clusters[p] != -1 && self.dst(self.pairs[i], self.pairs[p]) <= self.eps {
